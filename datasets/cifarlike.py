...
class CIFAR10(TVCIFAR10):
    ...
    def __init__(self, cfg, train=True, transform=None, target_transform=None, download=True):
        root = osp.join(cfg.VICTIM.DATA_ROOT, 'cifar10')
...
class CIFAR100(TVCIFAR100):
    ...
    def __init__(self, cfg, train=True, transform=None, target_transform=None, download=False):
        root = osp.join(cfg.VICTIM.DATA_ROOT, 'cifar100')
        super().__init__(root, train, transform, target_transform, download)
...
class SVHN(TVSVHN):
    def __init__(self, cfg, train=True, transform=None, target_transform=None, download=False):
        root = osp.join(cfg.VICTIM.DATA_ROOT, 'svhn')
...
class TinyImagesSubset(ImageFolder):
    ...
    def __init__(self, cfg, train=True, transform=None, target_transform=None):
        root = osp.join(cfg.VICTIM.DATA_ROOT, 'tiny-images-subset')
